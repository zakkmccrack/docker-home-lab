...
const QueueSource = Object.freeze({ LIBRARY: "library", ALBUM: "album", PLAYLIST: "playlist" });
...
function encodeCoverPath(rawPath) {
  return rawPath
    .split("/")
    .map(seg => encodeURIComponent(seg).replace(/'/g, "%27").replace(/\(/g, "%28").replace(/\)/g, "%29"))
    .join("/");
}
...
function buildLibraryQueue(startIndex) {
  ...
  if (state.randomize) {
    const rest = Array.from({ length: 49 }, () => Math.floor(Math.random() * max));
...
function playAlbum(bandIndex, albumName) {
  ...
  const indices = songs.map(s => state.library.findIndex(l => l.id === s.id));
...
function renderSongList() {
  ...
  state.library.forEach((song, i) => frag.appendChild(makeSongRow(song, () => selectSong(i))));
...
function highlightActiveRow() {
  document.querySelectorAll("#song-list tr.active").forEach(tr => tr.classList.remove("active"));
...
function setActiveFilterCol(activeId) {
  sortColumns.forEach(id => {
    document.getElementById(id)?.classList.toggle("active-filter", id === activeId);
  });
}
...
function orderByTitle() { orderBy("title", (a, b) => a.album.localeCompare(b.album) || a.title_full.localeCompare(b.title_full), "title-col"); }
function orderByArtist() { orderBy("artist", (a, b) => a.artist.localeCompare(b.artist), "artist-col"); }
function orderByAlbum() { orderBy("album", (a, b) => a.album.localeCompare(b.album) || a.title_full.localeCompare(b.title_full), "album-col"); }
function orderByTime() { orderBy("duration", (a, b) => a.duration - b.duration, "duration-col"); }
